...
let audioFileFormatCommandArgs = [
    "ogg",
    "m4a"
];
...
let Helper = {
    ...
    isValidCommand(command) {
        return processArgs.some(i => i.indexOf(command) >= 0);
    },

    getOptions() {
        
        audioFileFormatCommandArgs.forEach(args => {
            if(this.isValidCommand(`--audioFileFormat=${args}`)) {
                options.audioFileFormat = `.${args}`;
            }
        });  
        if(this.isValidCommand("--remainTree=true")) {
            options.remainTree = true;
        }
        if(this.isValidCommand("--remainTree=false")) {
            options.remainTree = false;
        }        
    }
};
...
class PluginConfiguration {
    ...
    readPluginFiles() {
        let files = fs.readdirSync(path.join(sourceDir, "js", "plugins"), 'utf8');        
        let configPath = path.join(sourceDir, "js", "plugins.js");
        ...
        console.log(locale.getString("NUM_OF_PLUGINS", this._length));

        this._plugins.forEach((plugin, i) => {
            plugin.readComments((pluginName, noteParams) => {
                
                console.log(locale.getString("COMPLETED_TO_PARSE", pluginName));
                
                this._noteParams = this._noteParams.concat(noteParams);
                
                console.log(locale.getString("CONCAT_NOTE_PARAMS", JSON.stringify(noteParams)));
                
                this._pendingDone[i] = true;

                if(this.isReady()) {
                    console.log(locale.getString("COMPLETED_PARSING_ALL_PLUGINS"));
...
class Database {
    ...
    static loadDataFile(name) {
        ...
        let json = fs.readFileSync(path.join(sourceDir, filepath), "utf8");

        let items = jsonParse(json);

        if(!items) {
            throw new Error(locale.getLocale("CANNOT_PARSE_ERROR", filepath));
        }
    ...
    static loadDataFileFaster(name) {
        ...
        let items = require(path.join(sourceDir, filepath));
...
module.exports.run = function(args) {
    ...
    const config = new PluginConfiguration((noteParams) => {
        ...
        //     Database.getFiles(targetFiles, targetDir);
        // }
        ...
        const utils = {
            ...
            flush(src, dst) {
                ...
                switch(this.getStatus(src, dst)) {
                    case 1:
                        console.log(locale.getString("GET_STATUS_1", src, dst));
                        break;
                    case 2:
                        console.log(locale.getString("GET_STATUS_2", dst));
                        return false;                    
                    case 3:
                        console.log(locale.getString("GET_STATUS_3", src, dst));
                        break;                    
                }
        ...
        let preTargetFiles = [];
        let afterTargetFiles = [];

        if(fs.pathExistsSync(targetDir)) {
            utils.getFiles(preTargetFiles, targetDir);
        }
        ...
        if(options.remainTree) {
            fs.copySync(rootPath, targetPath, {overwrite: true, filter: (src, dst) => {
                ...
                if(["img", "audio", ".vscode", ".git"].includes(path.dirname(path.relative(rootPath, src)))) {
                    return false;
                }
                ...
                if(resource) {
                    ...
                    resource._data.forEach(file => {
                        ...
                        fs.copySync(sourcePath, copyPath, {overwrite: true, filter: (src, dst) => {
                            ...
                            console.log(locale.getString("COPY_FILE", src, dst));
                            return true;
                        }});
        ...
        if(fs.pathExistsSync(targetDir)) {
            utils.getFiles(afterTargetFiles, targetDir);
        }

        const gap = Math.abs(afterTargetFiles.length - preTargetFiles.length);
        if(gap < options.maxGap) {
            afterTargetFiles.forEach((e, i, a) => {
                let filename = path.relative(targetDir, e).replace("\\", "/");
                let fullPath = path.join(sourceDir, filename);
    
                if(e === preTargetFiles[i]) {
                    if(!fs.existsSync(fullPath)) {
                        console.log(locale.getString("DETECTED_DELETED_FILE", path.basename(e)));
                        fs.unlinkSync(e);
                    }
                } else {
                    console.log(locale.getString("DETECTED_NEW_FILE", path.basename(e)));
                }
    
            });
        }

        console.log(locale.getString("DONE"));
